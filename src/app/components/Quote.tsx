...
export default function Quote() {
    ...
    if (!quote) return <p className='flex flex-col justify-center items-center'>Cargando Notas Random...</p>
    ...
    return (
        <section className={`flex flex-col justify-center items-center transition-opacity ease-in-out duration-1000 ${fade ? "opacity-0" : "opacity-100"}`}>
            ...
            <div className="flex flex-col justify-center items-center">
                ...
                <p className={`text-2xl text-center mt-4 ${poppins.className}`}>Autor: {quote.author}</p>
                <p className={`text-2xl text-center mt-4 ${poppins.className}`}>Categoria: {quote.category}</p>
            </div>
